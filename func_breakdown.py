...
("F01-01", [
 ...
 ("SeaTunnel批量导入作业", "作业配置生成器、分片/断点续传/限速执行", 0, .5, 2.5, 1, .5),
...
("F03-01", [
 ...
 ("处置引擎", "到期扫描、豁免检查、DROP PARTITION、VACUUM物理清除", 0, 0, 3, 1, .5),
...
("F05-05", [
 ("预览服务", "格式识别、图片/PDF/Office转换", 0, 0, 2, .5, .25),
...
("N4", "【非功能】部署与运维支撑", [
 ("Azure DevOps连接K8s与CI/CD部署流水线", "Service Connection/命名空间RBAC授权、构建→镜像→部署K8s流水线（测试/生产两套）、生产环境审批门禁", 0, 1.25, .5, 0, .25),
...
("N5", "【非功能】开发侧文档（架构/设计/接口）", [
 ...
 ("数据设计与ETL/作业文档", "分层设计/数据字典/ETL与作业说明（DevB提供）", 0, 0, 1.5, 0, 0),
]),
]
...
note = ("对照说明（四个口径）：① 功能直估 %s 人天（BA %s / Wade %s / DevB %s / 测试 %s / SLC %s）——已按配置界面/动态表单复杂度上调前端工作量；"
        ...
        "④ 合计 %s 人天 vs 计划185：Wade %s（计划60，基本持平）；DevB %s（超计划60约%s）；Mandy三职 %s（BA %s + 测试 %s + SLC %s，超计划50约%s）——角色级差异的吸收路径见下方「AI Native提效折算对照」。"
...
# ---------- 工作量合理性复查（逐项过检·不以计划170/185反推） ----------
CAL = 62  # 9/14~12/16 工作日（已扣国庆10/1-7、中秋9/25、周末；备用赶工日9/20、10/10不计）
mandy_g2 = round(gt[0] + gt[3] + gt[4], 2)
def _h(t):
    disp = sum(2 if ord(ch) > 127 else 1 for ch in t)
    import math
    return max(16, 15 * math.ceil(disp / 160))
rv = [
    ("工作量合理性复查（逐项过检·不以计划170/185反推）：", True),
    ...
    ("· 人×日历匹配（直估÷%d）：Wade %s＝%.0f%%；DevB %s＝%.0f%%⚠超出日历%.1f人天；Mandy三职直估%s＝%.0f%%（折算≈%s＝%.0f%%）；PM Mark 15。" % (CAL, gt[1], gt[1] / CAL * 100, gt[2], gt[2] / CAL * 100, gt[2] - CAL, mandy_g2, mandy_g2 / CAL * 100, mandy_adj, mandy_adj / CAL * 100), False),
    ("· 可完成性结论：总量可以完成，但须满足三个前提——①DevB的AI提效×0.85须兑现（ETL/SQL/作业配置AI生成有效落地；否则溢出约%.1f人天：先启用备用赶工日2天，再将F01-02文件包解析、F05-05 Office转换移Wade或降级）；②Wade负载%.0f%%无缓冲，N5开发侧文档须AI成文；③Mandy测试×0.65依赖AI用例生成在9/30首批（2.3.3）验证落地。" % (gt[2] - CAL, gt[1] / CAL * 100), False),
    ("· B计划（前提不兑现时按序降级范围，不影响12/16交付）：F01-02离线介质登记→上线后首月补充；F05-05预览降级为PDF/图片两类（Office转换延后）；F03-01 VACUUM物理清除→上线后首个运维窗口执行。", False),
]
rr2 += 1  # 空一行
for text, bold in rv:
    c = wsn.cell(row=rr2, column=1, value=text)
    c.font = Font(bold=bold, size=9, color="C00000")
    c.alignment = Alignment(wrap_text=True, vertical="center")
    wsn.merge_cells(start_row=rr2, start_column=1, end_row=rr2, end_column=12)
    wsn.row_dimensions[rr2].height = _h(text)
    rr2 += 1

# ---------- 使用说明（幂等） ----------
ws0 = wb["使用说明"]
desc = ("子任务级直估：模块→功能→子任务三层结构（模块列与功能列均为纵向合并单元格），%d项功能×%d子任务 + 非功能与工程支撑×%d子任务"
        "（N1框架初始化与基座/N2 ESLint·SonarLint·SAST代码质量治理/N3性能·鉴权·可观测·容量/N4部署运维），"
        "按BA/开发A/开发B/测试/SLC文档独立估人天；底部附与计划口径对照、AI提效折算与工作量合理性复查"
        % (len(B), sum(len(s[1]) for s in B), sum(len(s[2]) for s in N)))
fv_seen = 0
