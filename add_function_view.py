...
# ---------- 公共支撑（挂新WBS组编码） ----------
PUBLIC = [
 ("G1", "需求与SRS（跨功能）", {"BA": ["2.1", "2.2", "2.4"], "测试": ["2.3"]},
  {"BA": "业务访谈/26项澄清/SRS生成评审M1/需求贯穿", "测试": "NFR可执行化/测试计划/AI用例首批试点"}),
 ("G2", "工程初始化与部署基座（N1+N4-1/2/3）", {"Wade": ["3.3", "3.4"], "DevB": ["3.3", "3.4"], "测试": ["3.3", "3.4"], "文档": ["3.3"]},
  {"Wade": "仓库/规范/前端骨架/四道门禁CI-CD/部署流水线/K8s清单", "DevB": "后端骨架/集群对接/测试环境先期部署", "测试": "基座与门禁验证/测试环境验证", "文档": "工程基座SLC"}),
 ("G3", "架构与数据设计", {"Wade": ["3.1"], "DevB": ["3.2"]},
 ...
 ("G9", "项目管理（Mark）", {"PM": ["1"], "文档": ["1.2"]},
  {"PM": "启动会/周例会/每日同步/进度风险跟踪/里程碑评审/收尾", "文档": "会议纪要与项目文档整理"}),
]
...
for rr in range(1, ws0.max_row + 1):
    if ws0.cell(row=rr, column=1).value == "功能视角分解":
        ws0.cell(row=rr, column=2).value = ("按功能维度查看：26项功能×（BA/开发A/开发B/测试/文档）五类角色各自的工作内容与人天（V2.0=「功能拆解估算」直估原值），"
                                            "另有9类公共支撑行（需求/基座/架构/集成/测试/专项/上线/管理，挂新WBS组编码）；底部与WBS分解V2.0合计297.5人天自动对账")
        seen = True
if not seen:
    last = ws0.max_row + 1
